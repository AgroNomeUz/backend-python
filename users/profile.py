    ...
    if user.organization_id:
        ...
        if org_obj.region:
            region = {
                ...
                # The app keys its map off `soato` and its links off `slug`
                # (`api.schemas.RegionOut`). Omitted here they would fall back
                # to the schema defaults — `""` and `null` — which is drift
                # this builder exists to prevent, not an absent region.
